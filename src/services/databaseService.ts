...
export const cajaService = {
  ...
  async getAllMovimientos(userId: string) {
    const { data, error } = await supabase
      ...
      .select("id, user_id, tipo, categoria, monto, descripcion, referencia_id, referencia_tipo, fecha, created_at")
      .eq("user_id", userId)
...
export const reporteService = {
  ...
  async getAllPagos(userId: string) {
    const { data, error } = await supabase
      .from("pagos")
      .select("id, user_id, prestamo_id, monto_pagado, capital_abonado, interes_pagado, fecha_pago, numero_cuota, metodo_pago, notas, created_at")
      .eq("user_id", userId)

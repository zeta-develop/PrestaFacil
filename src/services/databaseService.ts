...
export const pagoService = {
  ...
  /**
   * Elimina el último pago registrado de un préstamo y revierte todos los
   * efectos contables asociados (saldo, cuotas, capital recuperado, interés
   * ganado, estado y capital_config).
   */
  async eliminarUltimoPago(userId: string, prestamoId: string) {
    // 1. Obtener el préstamo con sus pagos
    ...
    if (!prestamo.pagos || prestamo.pagos.length === 0) {
      throw new Error("No hay pagos registrados para eliminar");
    }

    // 2. Determinar el último pago: mayor número de cuota; desempate por fecha más reciente
    const ultimoPago = [...prestamo.pagos].sort((a, b) => {
      const cuotaA = Number(a.numero_cuota) || 0;
      const cuotaB = Number(b.numero_cuota) || 0;
      if (cuotaA !== cuotaB) return cuotaB - cuotaA;
      return (
    ...
    const montoPago = Number(ultimoPago.monto_pagado) || 0;
    const capitalAbonado = Number(ultimoPago.capital_abonado) || 0;
    const interesPagado = Number(ultimoPago.interes_pagado) || 0;

    // 3. Revertir los valores del préstamo
    const nuevoSaldo = Number(prestamo.saldo_pendiente) + montoPago;
    const nuevasCuotasPagadas = Math.max(0, Number(prestamo.cuotas_pagadas) - 1);
    const nuevoCapitalRecuperado = Math.max(0, Number(prestamo.capital_recuperado) - capitalAbonado);
    const nuevoInteresGanado = Math.max(0, Number(prestamo.interes_ganado) - interesPagado);
    // Si el préstamo estaba pagado, vuelve a activo al eliminar el último pago
    ...
    if (configData) {
      await supabase
        .from("capital_config")
        .update({
          capital_disponible: Math.max(0, Number(configData.capital_disponible) - montoPago),
          capital_en_calle: Number(configData.capital_en_calle) + capitalAbonado,
          ganancia_total: Math.max(0, Number(configData.ganancia_total) - interesPagado),
          total_recuperado: Math.max(0, Number(configData.total_recuperado) - capitalAbonado),
        })
        .eq("user_id", userId);
    }
...
export const cajaService = {
  ...
  async getAllMovimientos(userId: string) {
    const { data, error } = await supabase
      ...
      .select("id, tipo, categoria, monto, descripcion, fecha, created_at, user_id")
      .eq("user_id", userId)
...
export const reporteService = {
  ...
  async getAllPagos(userId: string) {
    const { data, error } = await supabase
      .from("pagos")
      .select("id, user_id, prestamo_id, monto, monto_pagado, capital_abonado, interes_pagado, numero_cuota, metodo_pago, fecha_pago, notas, created_at")
      .eq("user_id", userId)

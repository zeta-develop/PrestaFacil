...
export const cajaService = {
  ...
  async getAllMovimientos(userId: string) {
    const { data, error } = await supabase
      ...
      .select("id, user_id, tipo, categoria, monto, descripcion, fecha, created_at")
      .eq("user_id", userId)
...
export const reporteService = {
  ...
  async getAllPagos(userId: string) {
    const { data, error } = await supabase
      .from("pagos")
      .select("id, user_id, prestamo_id, monto_pagado, capital_abonado, interes_pagado, numero_cuota, fecha_pago, metodo_pago, notas, created_at")
      .eq("user_id", userId)
...
export const rutaService = {
  ...
  async registrarPago(
    ...
    const { data: configData } = await supabase
      .from("capital_config")
      .select("id, capital_disponible, capital_en_calle, ganancia_total, total_recuperado")
      .eq("user_id", userId)
      .single();

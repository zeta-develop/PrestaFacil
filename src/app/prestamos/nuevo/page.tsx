      ...
      if (descontarDeuda && deudasActivas > 0 && prestamosPendientes.length > 0) {
        ...
        for (const prestamoAnterior of prestamosPendientes) {
          ...
          const { data: prestamoData } = await supabase
            .from("prestamos")
            .select("id, user_id, cliente_id, monto, porcentaje_interes, total_a_pagar, ganancia_esperada, tipo_pago, numero_cuotas, valor_cuota, cuotas_pagadas, saldo_pendiente, capital_recuperado, interes_ganado, fecha_inicio, fecha_fin_esperada, estado, created_at, updated_at")
